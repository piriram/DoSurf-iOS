...
class SceneDelegate: UIResponder, UIWindowSceneDelegate {
    ...
    private func configureIQKeyboardManager() {
        ...
        IQKeyboardManager.shared.keyboardDistance = 32
        IQKeyboardManager.shared.resignOnTouchOutside = true
    }
    ...
    func scene(_ scene: UIScene, willConnectTo session: UISceneSession, options connectionOptions: UIScene.ConnectionOptions) {
        ...
        let vc = SurfSessionsTestViewController()
        window?.rootViewController = vc
        window?.makeKeyAndVisible()
        
//        if UserInfoManager.shared.readAccessToken() != nil{
//            var controllers:[UIViewController] = []
//            for tab in TabbarType.allCases {
//                let vc = tab.viewController
//                let nav = UINavigationController(rootViewController: vc)
//                nav.tabBarItem = UITabBarItem(title: tab.title, image: UIImage(systemName:tab.imageName), tag: tab.tag)
