...
class CustomTabBarController: BaseTabBarController {
    ...
    private weak var dashboardProvider: (UIViewController & DashboardChartProviding)? // ✅ 대시보드 참조
    ...
    private func bindCenterButton() {
        ...
        customTabBar.centerButtonTapped
            .subscribe(onNext: { [weak self] in self?.handleCenterButtonTap() })
            .disposed(by: disposeBag)
        
        isRecordingScreenPresented
            .distinctUntilChanged()
            .subscribe(onNext: { [weak self] in self?.customTabBar.updateCenterButtonState(isSelected: $0) })
            .disposed(by: disposeBag)
    }
    ...
    private func showSurfEndOverlay() {
        ...
        overlay.onSurfEnd = { [weak self] in self?.endSurfing() }
        overlay.onCancel  = { [weak self] in self?.hideSurfEndOverlay() }
    ...
    private func hideSurfEndOverlay(completion: (() -> Void)? = nil) {
        guard let overlay = surfEndOverlay else { animateTabBarVisibility(hidden: false); completion?(); return }
    ...
    private func animateTabBarVisibility(hidden: Bool) {
        UIView.animate(withDuration: 0.3, delay: 0, options: [.curveEaseInOut]) {
            ...
            self.tabBar.transform = hidden ? CGAffineTransform(translationX: 0, y: self.tabBar.frame.height) : .identity
        }
    }
}
...
class CustomTabBar: UITabBar {
    ...
    private func setupCenterButtonDesign() {
        ...
        if #available(iOS 15.0, *) {
            ...
            config.titleTextAttributesTransformer = UIConfigurationTextAttributesTransformer { incoming in
                var out = incoming; out.font = .systemFont(ofSize: 10, weight: .medium); return out
            }
    ...
    private func bindCenterButtonEvents() {
        centerButton.rx.controlEvent(.touchDown)
            .subscribe(onNext: { [weak self] in self?.animateButtonPress(pressed: true) })
            .disposed(by: disposeBag)
        centerButton.rx.controlEvent([.touchUpInside, .touchUpOutside, .touchCancel])
            .subscribe(onNext: { [weak self] in self?.animateButtonPress(pressed: false) })
            .disposed(by: disposeBag)
        centerButton.rx.tap
    ...
    override func hitTest(_ point: CGPoint, with event: UIEvent?) -> UIView? {
        if isHidden || alpha < 0.01 || window == nil { return nil }
        if let superBounds = superview?.bounds, frame.minY >= superBounds.height - 1 { return nil }
        let centerFrame = centerButton.frame.insetBy(dx: -10, dy: -10)
        if centerFrame.contains(point) { return centerButton }
    ...
    override func layoutSubviews() {
        ...
        if #available(iOS 13.0, *) { centerButton.layer.cornerCurve = .continuous }
    }
    override func sizeThatFits(_ size: CGSize) -> CGSize {
        var s = super.sizeThatFits(size); s.height += 20; return s
    }
    private func arrangeTabBarItems() {
        ...
        for subview in subviews where String(describing: type(of: subview)) == "UITabBarButton" {
            let newFrame: CGRect
            if buttonIndex == 0 {
                newFrame = CGRect(x: 0, y: subview.frame.origin.y, width: buttonWidth, height: subview.frame.height)
            } else {
                newFrame = CGRect(x: buttonWidth * 2, y: subview.frame.origin.y, width: buttonWidth, height: subview.frame.height)
            }
